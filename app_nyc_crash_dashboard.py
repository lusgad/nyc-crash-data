...
# Precompute optimized data structures for fast filtering
class CrashDataManager:
    def __init__(self, df):
        self.df = df.copy()
        self.precompute_aggregates()
        
    def precompute_aggregates(self):
        """Precompute common aggregates for fast access"""
        print("🔄 Precomputing aggregates for fast queries...")
        ...
        if "FULL ADDRESS" not in self.df.columns:
            self.df["FULL ADDRESS"] = self.df.get("ON STREET_NAME", "").fillna("") + ", " + self.df.get("BOROUGH", "")
        ...
        self.unique_boroughs = sorted(self.df["BOROUGH"].dropna().unique())
        self.unique_person_types = sorted(self.df["PERSON_TYPE"].dropna().unique())
        self.unique_injuries = sorted(self.df["PERSON_INJURY"].dropna().unique())
        
        # Precompute vehicle types and factors
        self.df["VEHICLE_TYPES_LIST"] = self.df.get("ALL_VEHICLE_TYPES", "").apply(self.parse_vehicle_list)
        all_vehicle_types_flat = [vt for sub in self.df["VEHICLE_TYPES_LIST"] for vt in sub]
        self.unique_vehicles = sorted({vt for sub in self.df["VEHICLE_TYPES_LIST"] for vt in sub})
        
        self.df["FACTORS_LIST"] = self.df.get("ALL_CONTRIBUTING_FACTORS", "").apply(self.parse_factor_list)
        self.unique_factors = sorted({f for sub in self.df["FACTORS_LIST"] for f in sub})
        
        print("✅ Precomputation complete!")

    def parse_vehicle_list(self, v):
        if pd.isna(v):
            return []
    ...
    def get_filtered_data(self, year_range=None, boroughs=None, vehicles=None, 
                         factors=None, injuries=None, person_type=None, search_text=None):
        """Fast filtering using precomputed data"""
        dff = self.df.copy()
        ...
        # Vehicle filter
        if vehicles:
            mask = dff["VEHICLE_TYPES_LIST"].apply(
                lambda lst: any(v in (lst if isinstance(lst, list) else []) for v in vehicles)
            )
            dff = dff[mask]
        ...
        if search_text:
            ...
            if "gender" in parsed:
                dff = dff[dff["PERSON_SEX"].isin(parsed["gender"])]

        return dff

# Initialize data manager
data_manager = CrashDataManager(df)

# Define consistent borough colors
BOROUGH_COLORS = {
...
# Year slider marks
min_year = int(data_manager.df["YEAR"].min()) if not data_manager.df["YEAR"].isna().all() else 2010
max_year = int(data_manager.df["YEAR"].max()) if not data_manager.df["YEAR"].isna().all() else pd.Timestamp.now().year
         ...
         # Year Range Slider - Full Width
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id="borough_filter",
                        options=[{"label": b, "value": b} for b in data_manager.unique_boroughs],
                        multi=True,
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id="vehicle_filter",
                        options=[{"label": v, "value": v} for v in data_manager.unique_vehicles],
                        multi=True,
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id="factor_filter",
                        options=[{"label": f, "value": f} for f in data_manager.unique_factors],
                        multi=True,
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{"label": v, "value": v} for v in data_manager.unique_person_types],
                        multi=True,
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id="injury_filter",
                        options=[{"label": i, "value": i} for i in data_manager.unique_injuries],
                        multi=True,
...
def update_dashboard(n_clicks, year_range, boroughs, vehicles, factors, injuries, person_type, search_text):
     # Use the optimized data manager for fast filtering
     dff = data_manager.get_filtered_data(
